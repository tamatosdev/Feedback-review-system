...
test('insertFeedbackRequest stores a token and findFeedbackRequestByToken looks it up', () => {
  const client = insertClient({ name: 'Acme Corp', email: 'billing@acme.com', service_type: 'Branding Package' });
  const token = crypto.randomUUID();

  const { created, row } = insertFeedbackRequest({ client_id: client.id, month: '2026-08', token });
  assert.strictEqual(created, true);
  assert.strictEqual(row.submitted, 0);
  assert.strictEqual(row.month, '2026-08');

  const found = findFeedbackRequestByToken(token);
  assert.ok(found, 'token should be found');
  assert.strictEqual(found.id, row.id);
  assert.strictEqual(found.client_id, client.id);

  assert.strictEqual(findFeedbackRequestByToken('no-such-token'), undefined);

  const marked = markFeedbackRequestSubmitted(row.id);
  assert.strictEqual(marked, true);
  assert.strictEqual(findFeedbackRequestByToken(token).submitted, 1);
});

test('feedback_requests is idempotent per (client, month) — no duplicate row', () => {
  const client = insertClient({ name: 'Beta Ltd', email: 'hello@beta.com' });
  const firstToken = crypto.randomUUID();
  const secondToken = crypto.randomUUID();

  const first = insertFeedbackRequest({ client_id: client.id, month: '2026-09', token: firstToken });
  assert.strictEqual(first.created, true);

  const second = insertFeedbackRequest({ client_id: client.id, month: '2026-09', token: secondToken });
  ...
  assert.strictEqual(findFeedbackRequestByToken(secondToken), undefined, 'second token must never be stored');
});

test('listActiveClients returns only active clients', () => {
  insertClient({ name: 'Active One', email: 'a@one.com' });
  insertClient({ name: 'Inactive One', email: 'i@one.com', status: 'inactive' });
...
test('queryFeedbackByMonth filters by YYYY-MM range', () => {
  const client = insertClient({ name: 'Gamma Co', email: 'g@co.com' });
  insertFeedback({
  ...
  const inRange = queryFeedbackByMonth({ fromMonth: '2026-03', toMonth: '2026-06' });
